...
def softmax(x): exp_x = np.exp(x - x.max(1, keepdims=True)); return exp_x / exp_x.sum(1, keepdims=True)

class MLP:
    def __init__(self, input_size, hidden_size, output_size, activation, lr=0.01):
    ...
    def forward(self, X):
        self.a1 = self.activation(X @ self.w1 + self.b1)
        return softmax(self.a1 @ self.w2 + self.b2)
    ...
    def train(self, X, y, epochs=1000):
        for _ in range(epochs):
            ...
            for w, b, dz in [(self.w2, self.b2, dz2), (self.w1, self.b1, dz1)]:
                w -= self.lr * (self.a1 if w is self.w2 else X).T @ dz
                b -= self.lr * dz.mean(0)
